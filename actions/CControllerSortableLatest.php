...
/**
 * Base controller for the "Sortable Latest data" page and the "Sortable Latest data" asynchronous refresh page.
 */
abstract class CControllerSortableLatest extends CController {
	...
	protected function prepareData(array $filter, $sort_field, $sort_order) {
		...
		if ($select_items) {
			...
			else if ($sort_field === 'lastvalue') {
				// Do nothing, because this time is Last value not available.
			}
			else {
				CArrayHelper::sort($items, [[
					'field' => 'name',
